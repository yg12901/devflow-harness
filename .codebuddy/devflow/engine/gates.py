...
1. **门禁是配置，不是代码。** 只实现约 10 个通用校验原语，具体检查
   什么写在 workflow.yaml 里，换技术栈不用碰引擎。

2. **门禁独立复算，不信 Agent 自述。** 校验编译是否成功不能靠在 markdown
   里 grep 成功字样 —— 那行字是 LLM 自己写的。command_ok 原语会真正把
   profile 里的构建/测试命令重跑一遍，用退出码说话。
"""
...
# ============================================================
# 需求覆盖、孤立任务、依赖成环、核心文件认领等全是确定性的图/集合运算，
# 交给脚本既快又稳，不会因为模型波动而漏判。
